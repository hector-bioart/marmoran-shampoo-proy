...
def analizar_espuma_repositorio():
    ...
    ruta_csv = os.path.join(ruta_raiz, 'data/4.4_propiedades_fisicoquimicas/resultados_espuma.csv')
    ...
    # Limpiamos nombres de columnas y las renombramos
    df.columns = df.columns.str.strip()
    df.columns = ['Formulacion', 'Replica', 'altura_espuma', 'colapso_espuma']

    # 3. VARIABLES A ANALIZAR (con sus unidades para la gráfica)
    variables = {
        'altura_espuma': 'cm', 
        'colapso_espuma': 'minutos'
    }

    print("\n" + "="*65)
    print("      ANÁLISIS DE DESEMPEÑO (ÍNDICE Y COLAPSO DE ESPUMA)")
    print("="*65)
    ...
    for var, unidad in variables.items():
        ...
        # Le damos un formato más amigable al nombre de la variable para imprimir
        nombre_var_impreso = var.replace("_", " ").title()

        print(f"\nANÁLISIS DE: {nombre_var_impreso.upper()}")
        ...
        if p_valor < 0.05:
            ...
            for fila in datos_tukey:
                ...
                if (g1 == 'Comercial' and g2 == 'F4') or (g1 == 'F4' and g2 == 'Comercial'):
                    ...
                    if not reject:
                        print(f"   ✔️ El desempeño de F4 es ESTADÍSTICAMENTE IGUAL al Comercial en {nombre_var_impreso}.")
                    else:
                        print(f"   ❌ El desempeño de F4 es DIFERENTE al Comercial en {nombre_var_impreso}.")
        else:
            print("   ⚠️ No hay diferencias significativas en esta prueba.")

        # 5. GRÁFICA
        plt.figure(figsize=(8, 5))
        sns.set_style("whitegrid")
        
        # Paleta 'ocean' para dar un contraste fresco a las gráficas de espuma
        sns.barplot(x='Formulacion', y=var, data=df, hue='Formulacion', 
                    palette='ocean', legend=False, errorbar='sd', capsize=.1)
        ...
        plt.title(f'Desempeño de {nombre_var_impreso}')
        plt.ylabel(f'{nombre_var_impreso} ({unidad})')
        plt.legend()
        
        # Guardado en la carpeta de resultados correcta
        nombre_img = f'espuma_{var}.png'
